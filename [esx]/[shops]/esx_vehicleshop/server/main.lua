    ...
    function (result)
      ...
      for i=1, #result, 1 do
        table.insert(vehicles, {
          ...
          rentedAt   = result[i].created_at
        })

        TriggerClientEvent('esx:showNotification', source, 'OS is ' .. os.date())
        TriggerClientEvent('esx:showNotification', source, 'Rented at ' .. os.date('%c', result[i].created_at))
        TriggerClientEvent('esx:showNotification', source, 'Diff is ' .. os.date() - result[i].created_at)
      end
  ...
  MySQL.Async.fetchAll(
    ...
    function (result)
      if #result > 0 then
        ...
        local rentPrice = result[1].rent_price
        local rentedAt  = result[1].created_at
        ...
        local rentedTime = os.time() - rentedAt

        TriggerClientEvent('esx:showNotification', source, rentedTime)
